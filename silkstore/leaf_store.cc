...
class LeafStore::LeafStoreIterator : public Iterator {
 public:
  LeafStoreIterator(const ReadOptions& options, LeafStore* store)
      : ropts_(options), store_(store), leaf_it_(nullptr) {
  ...
  bool Valid() const override {
    return status_.ok() && leaf_it_ && leaf_it_->Valid() &&
           leaf_index_it_->Valid();
  }
  ...
  Status status() const override {
    ...
    if (s.ok()) {
      if (leaf_it_ == nullptr)
        s = Status::Corruption("Empty Leaf Reference");
      else
        s = leaf_it_->status();
    }
    return s;
 ...
 private:
  ...
  void OpenLeafIterator() {
    if (leaf_index_it_->Valid()) {
      ...
      if (leaf_it_ != nullptr) delete leaf_it_;
...
MiniRunIndexEntry::MiniRunIndexEntry(const Slice& data)
    : raw_data_(data), run_datasize_(0) {
...
MiniRunIndexEntry MiniRunIndexEntry::Build(uint32_t seg_no, uint32_t run_no,
                                           Slice block_index_data,
                                           Slice filter_data,
                                           size_t run_datasize,
                                           std::string* buf) {
...
std::string LeafIndexEntry::ToString() {
  ...
  for (size_t i = 0; i < entries.size(); ++i) {
    if (i > 0) res += ",";
    res +=
        std::to_string(entries.size() - i - 1) +
        "(seg_num=" + std::to_string(entries[i].GetSegmentNumber()) +
        ", run_no=" + std::to_string(entries[i].GetRunNumberWithinSegment()) +
        ",run_datasize=" + std::to_string(entries[i].GetRunDataSize()) + ")";
  }
  res += "]";
  return res;
}
...
Status LeafIndexEntryBuilder::ReplaceMiniRunRange(
    const LeafIndexEntry& base, uint32_t start, uint32_t end,
    const MiniRunIndexEntry& replacement, std::string* buf,
    LeafIndexEntry* new_entry) {
  if (start >= base.GetNumMiniRuns() || end >= base.GetNumMiniRuns())
    return Status::InvalidArgument("[start, end] not within bound of [0, " +
                                   std::to_string(base.GetNumMiniRuns()) + "]");
  buf->clear();
  ...
  base.ForEachMiniRunIndexEntry(processor,
...
Status LeafStore::Get(const ReadOptions& options, const LookupKey& key,
  ...
  if (it->Valid() == false) return Status::NotFound("");
  // TODO(yunxiao): Can get old data that contains a segment which has been
  // deleted.
  ...
  auto processor = [&, this](const MiniRunIndexEntry& minirun_index_entry,
                             uint32_t) -> bool {
    ++runs_searched;
    ...
    DeferCode c3([run]() {
      delete run;
    });  // todo fix memory leak: new run but not delelte
  ...
  index_entry.ForEachMiniRunIndexEntry(
      processor, LeafIndexEntry::TraversalOrder::backward);
...
Iterator* LeafStore::NewIteratorForLeaf(const ReadOptions& options,
                                        const LeafIndexEntry& leaf_index_entry,
                                        Status& s, uint32_t start_minirun_no,
  ...
  auto processor = [&, this](const MiniRunIndexEntry& minirun_index_entry,
                             uint32_t run_no) -> bool {
    if (start_minirun_no <= run_no && run_no <= end_minirun_no) {
      uint32_t seg_no = minirun_index_entry.GetSegmentNumber();
      Segment* seg = nullptr;
      s = seg_manager_->OpenSegment(seg_no, &seg);
      if (!s.ok()) return true;  // error, early return
      MiniRun* run;
      Block index_block(
          BlockContents{minirun_index_entry.GetBlockIndexData(), false, false});
...
Iterator* LeafStore::NewDBIterForLeaf(
  ...
  Iterator* internal_iter = NewIteratorForLeaf(
      options, leaf_index_entry, s, start_minirun_no, end_minirun_no);
