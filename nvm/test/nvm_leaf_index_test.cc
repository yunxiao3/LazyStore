...
void Bench() {
  ...
  endTime = clock();
  std::cout <<"kNumOps: " << kNumOps << " The Get time is: " <<(endTime -
  startTime) << "\n"; std::cout << " @@@@@@@@@ PASS #########\n";
*/
  std::cout << " ######### Begin Sequential Iterator Test ######## \n";
  startTime = clock();
...
void IterTest(){
    ...
    leveldb::DB* db_ = nullptr;
    // leveldb::silkstore::NvmemLeafIndex* db = new NvmemLeafIndex(Options(),
nullptr); leveldb::Status s =
leveldb::silkstore::NvmemLeafIndex::OpenLeafIndex(leveldb::Options(), "", &db_);
    assert(s.ok()==true);
    std::cout << " ######### IterTest  ######## \n";
...
"\n"; auto it = db_->NewIterator(); it->SeekToFirst(); auto mit = m.begin();
            while (it->Valid()){
...
void EmptyIter() {
  ...
  // nullptr);
  leveldb::Status s = leveldb::silkstore::NvmLeafIndex::OpenNvmLeafIndex(
      leveldb::Options(), "./nvm_leaf_test", &db_);
  assert(s.ok() == true);
...
void Recovey() {
  ...
  s = leveldb::silkstore::NvmLeafIndex::OpenNvmLeafIndex(
      leveldb::Options(), "./nvm_leaf_test", &db_);
  ...
  while (it->Valid()) {
    ...
    auto res_value = it->value();
    it->Next();
    // std::cout << "res_key: " << res_key.ToString() << " res_value: " <<
    // res_value.ToString() << "\n";
    count++;
  }
